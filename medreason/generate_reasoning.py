...
def process_sample(sample_id, max_retries=5, retry_interval=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"Processing sample id {sample_id}. Attempt {attempt + 1}/{max_retries}")

            # 始终传 primekg（DataFrame），不要传 G（Graph）！
            ...
            logger.error(f"Sample {sample_id} attempt {attempt + 1} skipped due to error: {e}")
            try:
                logger.error(f"Question: {sample.get('question', 'N/A')}")
                logger.error(f"Answer: {sample.get('answer', 'N/A')}")
            except Exception:
                pass
            # 如果还有重试机会，则等待一段时间
    ...
    return {
        ...
        "reasoning": f"No reasoning path found. Error after {max_retries} attempts.",
        "answer": sample.get('answer', 'N/A'),
        "options": sample.get('options', 'N/A'),
        "huatuo": sample.get('comparison', 'N/A')
    }

def get_finished_ids(result_file_name):
    finished_ids = set()
        ...
        with open(result_file_name, 'r') as f:
            for line in f:
                try:
                    ...
                    # id 可能是 int 或 str，建议统一为 int
                    finished_ids.add(int(data.get("id")))
                except Exception:
                    continue
    return finished_ids
...
if __name__ == '__main__':
    ...
    nodeemb_dict = torch.load('node_embeddings.pt', weights_only=False)
    ...
    # ----------- 断点续传部分 -----------
    finished_ids = get_finished_ids(result_file_name)
    logger.info(f"Already processed sample ids: {sorted(finished_ids)}")
    to_process_ids = [ids for ids in range(args.start_idx, args.start_idx + test_samples) if ids not in finished_ids]
    logger.info(f"Total to process: {len(to_process_ids)}")

    if args.batch_size == 1:
        with open(result_file_name, 'a') as f:  # 注意改为'a'模式，追加写入
            for ids in tqdm(to_process_ids):
                ...
                try:
                    ...
                    logger.error(f"Sample {ids} skipped due to error: {e}")
                    error_samples.append(ids)
        ...
        logger.info(f"Total processed: {len(to_process_ids) - len(error_samples)}")
        logger.info(f"Total errors: {len(error_samples)}")
        logger.info(f"Error sample indices: {error_samples}")
        ...
        ) as pool, open(result_file_name, 'a') as f:  # 注意改为'a'模式，追加写入
            results = []
            for res in tqdm(pool.imap(process_sample, to_process_ids), total=len(to_process_ids)):
                if res is not None and "No reasoning path found. Error:" in str(res.get("reasoning", "")):
                    error_samples.append(res["id"])
                results.append(res)
