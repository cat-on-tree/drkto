...
def generate_node_embeddings(knowledge_graph_path = 'data/kg.csv', emb_model_name = 'abhinand/MedEmbed-large-v0.1'):
    knowledge_graph = pd.read_csv(knowledge_graph_path, low_memory=False)
    emb_model = SentenceTransformer(emb_model_name).to('cpu')
...
def coarse_entity_extraction(text,temperature = 0.0, max_tokens = 3000, engine="qwen-max"):
...
def most_correlated_enetity_selection(question, query_entity, similar_entities, temperature = 0.0, max_tokens = 3000, engine="qwen-max"):
...
def QA_reformat_based_on_entity(question, answer, entity_list_text, temperature = 0.0, max_tokens = 5000, engine="qwen-max"):
    Reformat_prompt = """ You are a helpful, pattern-following medical assistant.
...
def QA_reformat_with_entity_extraction(question, answer, knowledge_graph, emb_model,nodeemb_dict, temperature = 0.0, max_tokens = 5000, engine="qwen-max"):
...
def path_sampling(path_all, question, answer, topK_reasoning_paths, max_path_number_per_group = 50, engine = "qwen-max", logger = None):
...
def most_correlated_path_selection(question, paths_text, answer, topK = 2, temperature = 0.0, max_tokens = 4000, engine="qwen-max"):
    Reformat_prompt = """ You are a helpful, pattern-following medical assistant.
...
def llm_judge_answer(llm_output, answer, engine='qwen-max'):
    prompt = f"""
